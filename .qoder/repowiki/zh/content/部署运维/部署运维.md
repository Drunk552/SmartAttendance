...
- [stability_test.sh](file://tools/stability_test.sh)
- [quick_stability_test.sh](file://tools/quick_stability_test.sh)
- [analyze_stability.py](file://tools/analyze_stability.py)
</cite>
...
## 项目结构
项目采用 CMake 构建，主程序入口位于 src/main.cpp，业务层封装在 src/business，数据层封装在 src/data，UI 层封装在 src/ui。LVGL 作为图形界面库通过子目录方式集成，配置文件为根目录 lv_conf.h。构建脚本与环境变量配置位于 env/env.sh，工具脚本位于 tools/。新增了稳定性测试框架，包含完整的内存监控、崩溃检测与报告生成功能。
...
- 稳定性测试框架：提供 1 小时与 10 分钟快速稳定性测试，包含内存监控、崩溃检测与报告生成。
...
## 架构总览
系统采用三层架构：UI 层（LVGL）、业务层（OpenCV + SQLite3）、数据层（SQLite3）。主程序负责初始化顺序与主循环，业务层负责摄像头与识别逻辑，数据层负责持久化与查询。新增稳定性测试框架提供自动化质量保障。
...
END
subgraph "测试工具"
PY["Python 分析脚本<br/>analyze_stability.py"]
SH["Shell 测试脚本<br/>stability_test.sh/quick_stability_test.sh"]
PS["PowerShell 推流<br/>stream.ps1"]
END
M --> UI
M --> BUS
M --> DATA
...
- [CMakeLists.txt:165-201](file://CMakeLists.txt#L165-L201)
...
- 稳定性测试：tools/stability_test.sh 每 5 秒记录 RSS 与内存占比，持续 1 小时，检测长时间运行稳定性。
- 快速测试：tools/quick_stability_test.sh 提供 10 分钟快速稳定性测试，用于开发阶段快速验证。
- 数据分析：tools/analyze_stability.py 提供内存使用趋势分析与可视化报告生成。
- 数据备份：SQLite3 数据库文件与抓拍图片目录需定期备份；可结合业务层提供的清理接口与工厂重置接口进行维护。
- 升级维护：通过 env/env.sh 的 make/m 重新构建，run/r 重启服务；Dockerfile 可用于标准化构建环境。

**章节来源**
- [main.cpp:229-238](file://src/main.cpp#L229-L238)
- [stability_test.sh:1-197](file://tools/stability_test.sh#L1-L197)
- [quick_stability_test.sh:1-106](file://tools/quick_stability_test.sh#L1-L106)
- [analyze_stability.py:1-205](file://tools/analyze_stability.py#L1-L205)
...
- 稳定性测试：Linux 环境下可直接运行稳定性测试脚本，Windows 环境下建议使用 PowerShell 脚本进行摄像头推流测试。
...
- 新增稳定性测试框架依赖 Python pandas、matplotlib 等库进行数据分析与可视化。
...
DBH --> SQ
DBH --> XL["xlsxwriter"]
TEST["stability_test.sh"] --> PY["analyze_stability.py"]
TEST --> SH["quick_stability_test.sh"]
```
...
- 稳定性问题：使用稳定性测试框架检测内存泄漏与性能退化，通过分析脚本生成可视化报告。
...
## 结论
本部署运维文档基于现有仓库配置与脚本，给出了从编译、部署到性能优化与故障排除的完整实践路径。新增的稳定性测试框架提供了自动化质量保障机制，建议在生产环境进一步完善交叉编译与静态链接策略、容器化构建流程，并建立自动化监控与备份机制，以提升系统的稳定性与可维护性。

## 附录
- 构建与运行快捷命令：env/env.sh 提供 make/m 与 run/r，一键完成构建与启动。
- 稳定性测试：tools/stability_test.sh 提供 1 小时稳定性测试，tools/quick_stability_test.sh 提供 10 分钟快速测试。
- 数据分析：tools/analyze_stability.py 提供内存使用趋势分析与可视化报告生成。
