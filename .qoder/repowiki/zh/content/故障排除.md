...
## 项目结构
项目采用分层架构：UI层（LVGL + SDL）、业务层（人脸识别、事件总线、规则引擎、报表生成）、数据层（SQLite + BLOB存储）。顶层入口负责系统初始化与主循环，CMake负责依赖解析与链接。
...
- 业务层：人脸识别采集与识别、异步打卡写库、事件总线、配置缓存与线程池、报表生成。
- 数据层：SQLite封装、表结构与索引、BLOB存储、事务与并发控制、数据播种。
- 配置与构建：LVGL配置、CMake依赖与链接、编译命令导出、稳定性测试目标。
...
**章节来源**
- [tools/stability_test.sh:1-20](file://tools/stability_test.sh#L1-L20)
...
- 压力测试
  - 建议：使用stability_test.sh进行长时间稳定性验证。
