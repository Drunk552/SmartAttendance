...
## 简介
本文件面向智能考勤系统，基于现有代码库，系统化梳理数据库设计与实现，涵盖表结构、字段定义、数据类型选择、实体关系模型、外键约束、DAO 模式与事务处理、数据迁移与版本管理策略、性能优化与安全备份方案。**更新内容**：新增多公司架构支持，包括companies表、departments表的company_id外键关系，以及用户表face_data字段重命名为face_feature。**新增功能**：批量排班管理、全面数据清理、部门播种过程改进。
...
## 架构总览
数据库层采用 SQLite 文件型数据库，通过 RAII 封装的语句对象与预编译语句提升安全性与性能；使用共享/排他锁实现并发控制；通过事务加速批量写入；通过外键约束与 ON DELETE 策略保障数据一致性。**新增**：支持多公司架构，每个部门必须归属于特定公司。**新增**：批量排班管理功能，支持U盘/网络批量同步员工排班。
...
## 结论
本数据库设计以 SQLite 为核心，结合预编译语句、事务与并发锁，实现了高性能、高可靠的数据访问。**更新内容**：新增多公司架构支持，通过companies表和departments表的company_id外键关系，支持企业级多公司部署。**新增功能**：批量排班管理功能支持U盘/网络批量同步员工排班，全面数据清理功能支持保留管理员和AUTOINCREMENT序列重置。通过外键约束与 ON DELETE 策略保障数据一致性，通过动态 ALTER 与 INSERT OR REPLACE 实现平滑升级。配合工厂重置、清空与磁盘清理等维护手段，满足智能考勤系统的日常运行与长期演进需求。
