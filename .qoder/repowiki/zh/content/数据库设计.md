...
## 更新摘要
**所做更改**
- 部门种子数据从3个扩展到16个，支持更大规模组织管理
- 默认部门命名从'Not Set'系列扩展到'Not Set 1-16'，提供更清晰的部门标识
- 查询排序逻辑从按名称排序改为按ID排序，提升查询性能和稳定性
- 新增多公司架构支持，包括新的companies表和公司管理接口
- 更新部门表结构，添加company_id外键关系
- 用户表face_data字段重命名为face_feature
- 新增部门-公司联合索引优化查询性能
- 完善公司管理DAO接口，支持CRUD操作
- **新增批量排班管理功能**：db_batch_update_user_schedules()支持批量更新员工排班
- **增强数据清理功能**：db_clear_all_employee_data()支持保留管理员和AUTOINCREMENT序列重置
- **改进部门播种过程**：在播种时重置AUTOINCREMENT序列计数器
- 更新实体关系模型以反映新的多公司架构
...
## 简介
本文件面向智能考勤系统，基于现有代码库，系统化梳理数据库设计与实现，涵盖表结构、字段定义、数据类型选择、实体关系模型、外键约束、DAO 模式与事务处理、数据迁移与版本管理策略、性能优化与安全备份方案。**更新内容**：新增多公司架构支持，包括companies表、departments表的company_id外键关系，以及用户表face_data字段重命名为face_feature。**新增功能**：批量排班管理、全面数据清理、部门播种过程改进。**重大变更**：部门种子数据扩展至16个，查询排序逻辑优化为按ID排序，支持更大规模组织的部门管理需求。

## 项目结构
数据库层位于 src/data，核心文件为 db_storage.cpp/.h，提供 SQLite 数据库的初始化、表结构创建与升级、DAO 接口与事务控制。业务层 attendance_rule.cpp/.h 通过 DAO 接口完成考勤规则计算与记录入库。UI层通过ui_controller.cpp调用批量导入功能。
...
- [ui_controller.cpp:845](file://src/ui/ui_controller.cpp#L845)
- [ui_controller.h:33](file://src/ui/ui_controller.h#L33)
...
## 架构总览
数据库层采用 SQLite 文件型数据库，通过 RAII 封装的语句对象与预编译语句提升安全性与性能；使用共享/排他锁实现并发控制；通过事务加速批量写入；通过外键约束与 ON DELETE 策略保障数据一致性。**新增**：支持多公司架构，每个部门必须归属于特定公司。**新增**：批量排班管理功能，支持U盘/网络批量同步员工排班。**重大优化**：部门查询排序逻辑从按名称排序改为按ID排序，提升查询性能和稳定性。
...
- **批量排班管理**：db_batch_update_user_schedules() 支持U盘/网络批量同步员工排班，使用INSERT OR REPLACE进行覆盖更新。
- **全面数据清理**：db_clear_all_employee_data() 支持保留管理员或完全清空数据，包含AUTOINCREMENT序列重置。
...
- **优化的部门查询**：db_get_departments() 和 db_get_departments_by_company() 使用 ORDER BY id 优化查询性能。

**章节来源**
- [db_storage.cpp:133-310](file://src/data/db_storage.cpp#L133-L310)
- [db_storage.cpp:432-486](file://src/data/db_storage.cpp#L432-L486)
- [db_storage.cpp:831-929](file://src/data/db_storage.cpp#L831-L929)
- [db_storage.cpp:1054-1115](file://src/data/db_storage.cpp#L1054-L1115)
- [db_storage.cpp:2293-2355](file://src/data/db_storage.cpp#L2293-L2355)
- [db_storage.cpp:1659-1788](file://src/data/db_storage.cpp#L1659-L1788)
...
- **AUTOINCREMENT序列管理**：在播种过程中重置部门表的AUTOINCREMENT序列，确保ID从1开始。
- **大规模部门管理**：部门播种从3个扩展到16个，支持更大规模组织的部门管理需求。
...
- [db_storage.cpp:385-386](file://src/data/db_storage.cpp#L385-L386)
...
### 部门播种与查询优化
**重大更新**：部门播种系统已从3个扩展到16个，默认部门命名从'Not Set'系列扩展到'Not Set 1-16'，查询排序逻辑从按名称排序改为按ID排序。

- **部门播种过程**：
  - 首先重置 AUTOINCREMENT 序列计数器，确保ID从1开始
  - 插入必须存在的默认部门（ID: 1）：'Not Set 1'
  - 循环插入额外的15个部门（总计16条）：'Not Set 2'到'Not Set 16'
  - 每个部门都关联到默认公司ID（1）

- **查询排序优化**：
  - `db_get_departments()` 和 `db_get_departments_by_company()` 使用 `ORDER BY id` 替代 `ORDER BY name`
  - `db_get_all_departments_with_company()` 使用 `ORDER BY c.name, d.name` 保持公司和部门的层次排序
  - 按ID排序提升了查询性能，特别是在大量部门数据的情况下

- **支持更大规模组织**：
  - 16个默认部门足以支持中大型企业的基本组织架构
  - 每个部门都有唯一的ID标识，便于精确管理和查询
  - 支持按公司分组的部门管理，适应多公司架构

**章节来源**
- [db_storage.cpp:370-411](file://src/data/db_storage.cpp#L370-L411)
- [db_storage.cpp:524-566](file://src/data/db_storage.cpp#L524-L566)
...
- **批量导入依赖**：ui_controller.cpp 通过 db_batch_add_users() 和 db_batch_update_user_schedules() 实现批量数据导入。
- **部门查询依赖**：UI层通过 UiController::getDepartmentList() 获取部门列表，支持16个默认部门的完整管理。

```mermaid
sequenceDiagram
participant UI as "UI 控制器"
participant Import as "批量导入"
participant DAO as "数据层 DAO"
participant DB as "SQLite 数据库"
UI->>Import : "触发批量导入"
Import->>DAO : "db_batch_add_users()"
DAO->>DB : "批量插入员工数据"
DB-->>DAO : "返回成功"
DAO-->>Import : "返回成功"
Import->>DAO : "db_batch_update_user_schedules()"
DAO->>DB : "批量更新员工排班"
DB-->>DAO : "返回成功"
DAO-->>Import : "返回成功"
Import-->>UI : "导入完成"
...
- **AUTOINCREMENT序列优化**：在播种时重置序列计数器，避免ID浪费。
- **查询排序优化**：部门查询使用 ORDER BY id 替代 ORDER BY name，提升大量数据下的查询性能。
- **大规模部门管理**：16个默认部门的播种系统支持更大规模组织的部门管理需求。
...
- **批量排班失败**：检查用户ID是否存在，班次ID是否有效，日期格式是否正确。
- **数据清理失败**：确认保留管理员参数设置，检查文件系统权限。
- **公司管理问题**：检查 companies 表是否存在，确认默认公司ID是否正确。
- **部门播种失败**：检查 AUTOINCREMENT 序列重置是否成功，确认默认公司ID为1。
- **查询性能问题**：确认部门查询使用 ORDER BY id，而非 ORDER BY name。
...
## 结论
本数据库设计以 SQLite 为核心，结合预编译语句、事务与并发锁，实现了高性能、高可靠的数据访问。**更新内容**：新增多公司架构支持，通过companies表和departments表的company_id外键关系，支持企业级多公司部署。**重大优化**：部门种子数据扩展至16个，查询排序逻辑从按名称排序改为按ID排序，支持更大规模组织的部门管理需求。**新增功能**：批量排班管理功能支持U盘/网络批量同步员工排班，全面数据清理功能支持保留管理员和AUTOINCREMENT序列重置。通过外键约束与 ON DELETE 策略保障数据一致性，通过动态 ALTER 与 INSERT OR REPLACE 实现平滑升级。配合工厂重置、清空与磁盘清理等维护手段，满足智能考勤系统的日常运行与长期演进需求。
...
- **批量操作接口**：db_batch_update_user_schedules()、db_clear_all_employee_data()
- **部门管理接口**：db_get_departments()、db_get_departments_by_company()、db_get_all_departments_with_company()
- **默认部门数量**：16个（Not Set 1-16）
- **查询排序优化**：部门查询使用 ORDER BY id 提升性能
