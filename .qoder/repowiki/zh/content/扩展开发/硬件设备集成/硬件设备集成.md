# 硬件设备集成

<cite>
**本文档引用的文件**
- [main.cpp](file://src/main.cpp)
- [face_demo.h](file://src/business/face_demo.h)
- [face_demo.cpp](file://src/business/face_demo.cpp)
- [ui_app.h](file://src/ui/ui_app.h)
- [ui_app.cpp](file://src/ui/ui_app.cpp)
- [lv_conf.h](file://lv_conf.h)
- [lv_indev.h](file://libs/lvgl/src/indev/lv_indev.h)
- [lv_indev.c](file://libs/lvgl/src/indev/lv_indev.c)
- [lv_indev_private.h](file://libs/lvgl/src/indev/lv_indev_private.h)
- [lv_sdl_window.c](file://libs/lvgl/src/drivers/sdl/lv_sdl_window.c)
- [lv_port_disp_template.h](file://libs/lvgl/examples/porting/lv_port_disp_template.h)
- [lv_port_indev_template.h](file://libs/lvgl/examples/porting/lv_port_indev_template.h)
- [t9_keyboard.h](file://src/ui/common/t9_keyboard.h)
- [t9_keyboard.cpp](file://src/ui/common/t9_keyboard.cpp)
- [t9_keymap.h](file://src/ui/common/t9_keymap.h)
- [t9_keymap.cpp](file://src/ui/common/t9_keymap.cpp)
- [matrix_t9_keyboard.h](file://src/ui/common/matrix_t9_keyboard.h)
- [matrix_t9_keyboard.cpp](file://src/ui/common/matrix_t9_keyboard.cpp)
- [T9键盘集成指南.md](file://docs/markdowm/T9键盘集成指南.md)
</cite>

## 更新摘要
**变更内容**
- 新增T9键盘系统集成章节，涵盖硬件键盘映射、输入法支持和UI组件
- 更新输入设备集成部分，增加T9键盘作为硬件输入设备的说明
- 新增T9键盘映射表和矩阵键盘处理逻辑
- 更新硬件配置示例，包含T9键盘相关配置

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [依赖关系分析](#依赖关系分析)
7. [性能考虑](#性能考虑)
8. [故障排查指南](#故障排查指南)
9. [结论](#结论)
10. [附录](#附录)

## 简介
本指南面向智能考勤系统的硬件设备集成，聚焦以下目标：
- 摄像头设备集成：驱动适配、图像采集接口、视频流处理方案
- 输入设备集成：指纹识别器、IC卡读卡器、触摸屏等外设的驱动开发与接口封装，**新增T9键盘系统集成**
- 显示设备适配：多分辨率显示器支持、颜色格式转换、刷新率优化
- 硬件抽象层设计：设备接口标准化、设备状态管理、错误处理机制
- 硬件配置示例与调试方法：帮助开发者完成设备正确集成

## 项目结构
系统采用分层架构：
- 应用入口负责系统初始化、依赖自检与主循环调度
- UI层基于LVGL，提供显示与输入设备抽象，**新增T9键盘UI组件**
- 业务层负责摄像头采集、人脸检测与识别、考勤记录异步落库
- 数据层负责数据库访问与缓存

```mermaid
graph TB
A["应用入口<br/>src/main.cpp"] --> B["UI层<br/>src/ui/ui_app.cpp"]
B --> C["LVGL显示驱动(SDL)<br/>libs/lvgl/src/drivers/sdl/lv_sdl_window.c"]
B --> D["输入设备抽象<br/>libs/lvgl/src/indev/*.c"]
B --> K["T9键盘系统<br/>src/ui/common/t9_keyboard*.cpp"]
A --> E["业务层<br/>src/business/face_demo.cpp"]
E --> F["OpenCV视频采集<br/>GStreamer/USB摄像头"]
E --> G["SQLite3数据库<br/>数据落库"]
B --> H["事件总线/控制器<br/>src/ui/ui_controller.*"]
K --> I["T9键盘映射表<br/>src/ui/common/t9_keymap.cpp"]
K --> J["矩阵键盘处理<br/>src/ui/common/matrix_t9_keyboard.cpp"]
```

**图表来源**
- [main.cpp:187-246](file://src/main.cpp#L187-L246)
- [ui_app.cpp:34-94](file://src/ui/ui_app.cpp#L34-L94)
- [face_demo.cpp:557-694](file://src/business/face_demo.cpp#L557-L694)
- [lv_sdl_window.c:95-239](file://libs/lvgl/src/drivers/sdl/lv_sdl_window.c#L95-L239)
- [lv_indev.c:261-317](file://libs/lvgl/src/indev/lv_indev.c#L261-L317)
- [t9_keyboard.cpp:1-200](file://src/ui/common/t9_keyboard.cpp#L1-L200)
- [t9_keymap.cpp:1-128](file://src/ui/common/t9_keymap.cpp#L1-L128)
- [matrix_t9_keyboard.cpp:1-200](file://src/ui/common/matrix_t9_keyboard.cpp#L1-L200)

**章节来源**
- [main.cpp:187-246](file://src/main.cpp#L187-L246)
- [ui_app.cpp:34-94](file://src/ui/ui_app.cpp#L34-L94)
- [face_demo.cpp:557-694](file://src/business/face_demo.cpp#L557-L694)

## 核心组件
- 应用入口与主循环：负责系统初始化、依赖检查、UI与业务初始化、LVGL心跳与定时器
- UI层与显示驱动：使用LVGL SDL驱动创建窗口、设置刷新回调、绑定输入设备
- 业务层摄像头与识别：后台采集线程、人脸检测与识别、异步写库、事件发布
- 输入设备抽象：LVGL输入设备类型与读回调机制，支持指针/键盘/编码器/按钮，**新增T9键盘支持**
- LVGL配置：颜色深度、刷新周期、渲染配置等
- **T9键盘系统：提供硬件键盘映射、多模式输入法、候选词选择功能**

**章节来源**
- [main.cpp:187-246](file://src/main.cpp#L187-L246)
- [ui_app.cpp:34-94](file://src/ui/ui_app.cpp#L34-L94)
- [face_demo.cpp:557-694](file://src/business/face_demo.cpp#L557-L694)
- [lv_conf.h:29-95](file://lv_conf.h#L29-L95)

## 架构总览
系统采用"UI层 + 业务层 + 数据层"的分层设计，业务层通过事件总线与UI层解耦，摄像头采集与识别在后台线程运行，避免阻塞UI。**新增T9键盘系统作为硬件输入设备集成到系统中。**

```mermaid
sequenceDiagram
participant App as "应用入口(main)"
participant UI as "UI层(ui_app)"
participant Disp as "显示驱动(SDL)"
participant Bus as "事件总线"
participant Biz as "业务层(face_demo)"
participant Cam as "摄像头/视频流"
participant DB as "数据库"
participant T9 as "T9键盘系统"
App->>UI : 初始化UI
UI->>Disp : 创建窗口/设置刷新回调
UI->>Biz : 启动后台服务
Biz->>Cam : 打开视频源(GStreamer/USB)
Biz->>Biz : 后台采集线程循环
Biz->>Cam : 读取帧/检测/识别
Biz->>Bus : 发布 CAMERA_FRAME_READY
UI->>Disp : 触发刷新
Biz->>DB : 异步写入考勤记录
UI->>T9 : 处理硬件键盘事件
T9->>UI : 更新文本显示
```

**图表来源**
- [main.cpp:213-238](file://src/main.cpp#L213-L238)
- [ui_app.cpp:86-93](file://src/ui/ui_app.cpp#L86-L93)
- [face_demo.cpp:522-527](file://src/business/face_demo.cpp#L522-L527)
- [face_demo.cpp:1024-1036](file://src/business/face_demo.cpp#L1024-L1036)

## 详细组件分析

### 摄像头设备集成
- 驱动与适配
  - 业务层通过OpenCV VideoCapture打开视频源，支持GStreamer管道与USB摄像头
  - 硬编码GStreamer管道参数，适配RAW/YCbCr-4:2:2格式，输出BGR
  - 提供SDP重连逻辑，连续读取失败时强制释放并重连
- 图像采集接口
  - 后台线程持续读取帧，使用互斥锁保护共享帧，避免UI与采集线程竞争
  - 采集线程对帧进行人脸检测/识别，绘制框与文本，更新显示帧缓存
  - UI通过business_get_display_frame获取缩放后的RGB帧，用于显示
- 视频流处理
  - 跳帧策略：每5帧检测一次，其余帧复用上一次检测结果，降低CPU占用
  - 识别冷却：用户识别冷却时间控制，避免频繁重复打卡
  - 异步写库：识别结果入队，后台线程串行写库，避免SQLite并发冲突

```mermaid
flowchart TD
Start(["开始"]) --> CheckCap["检查视频源是否打开"]
CheckCap --> |否| Reopen["释放并重连GStreamer/USB"]
Reopen --> Sleep["短暂休眠"] --> CheckCap
CheckCap --> |是| ReadFrame["读取一帧"]
ReadFrame --> Empty{"帧为空?"}
Empty --> |是| FailCnt["连续失败计数+1"] --> Retry{"连续失败>60帧?"}
Retry --> |是| ForceRelease["强制释放并重连"] --> CheckCap
Retry --> |否| Sleep2["微休眠"] --> CheckCap
Empty --> |否| ResetFail["重置连续失败计数"]
ResetFail --> Detect{"是否检测帧?"}
Detect --> |是| FaceDetect["Haar检测最大人脸"]
Detect --> |否| Track["跟踪上一次人脸框"]
FaceDetect --> HasFace{"检测到人脸?"}
Track --> HasFace
HasFace --> |是| Preprocess["预处理(裁剪/缩放/均衡化)"]
Preprocess --> Predict["LBPH预测/阈值判断"]
Predict --> Valid{"有效识别?"}
Valid --> |否| DrawBox["绘制框(跳帧色)"] --> UpdateDisplay["更新显示帧缓存"]
Valid --> |是| Cooldown{"用户冷却中?"}
Cooldown --> |是| DrawBox
Cooldown --> |否| AntiDup["业务防抖(时间间隔)"]
AntiDup --> Queue["入队写库任务"]
Queue --> DrawBox
DrawBox --> UpdateDisplay
UpdateDisplay --> Sleep3["线程休眠(约15ms)"] --> CheckCap
```

**图表来源**
- [face_demo.cpp:312-549](file://src/business/face_demo.cpp#L312-L549)
- [face_demo.cpp:223-240](file://src/business/face_demo.cpp#L223-L240)
- [face_demo.cpp:1024-1036](file://src/business/face_demo.cpp#L1024-L1036)

**章节来源**
- [face_demo.cpp:223-240](file://src/business/face_demo.cpp#L223-L240)
- [face_demo.cpp:312-549](file://src/business/face_demo.cpp#L312-L549)
- [face_demo.cpp:1024-1036](file://src/business/face_demo.cpp#L1024-L1036)

### 输入设备集成
- LVGL输入设备抽象
  - 输入设备类型：指针、键盘、编码器、按钮
  - 读回调机制：每个输入设备注册读回调，在定时器中被周期性调用
  - 设备状态：按下/释放状态、时间戳、长按重复等
- SDL输入设备仿真
  - UI层创建SDL鼠标与键盘输入设备，绑定到UI组，实现键盘导航
- **T9键盘系统集成**
  - **硬件键盘映射**：提供T9键盘到PC键盘的映射表，支持4x4硬件键盘布局
  - **多模式输入法**：支持数字模式、英文模式、中文模式、符号模式
  - **候选词选择**：支持上下方向键导航，OK键确认选择
  - **矩阵键盘处理**：专门的矩阵键盘处理类，支持硬件矩阵键盘事件
- 外设扩展建议
  - 指纹识别器/IC卡读卡器/触摸屏：通过实现LVGL输入设备读回调接入
  - 读回调中将外设事件转换为LVGL输入数据结构，交由LVGL处理

```mermaid
classDiagram
class lv_indev_t {
+type
+read_cb(data)
+state
+mode
+timestamp
+disp
+read_timer
}
class lv_indev_data_t {
+state
+point
+key
+btn_id
+enc_diff
+timestamp
+continue_reading
}
class T9Keyboard {
+init()
+handleKeyPress(key)
+switchInputMode(mode)
+getCurrentText()
+getCandidates()
}
class T9KeyMap {
+init()
+findT9KeyByPCKeycode(pc_keycode)
+getPCKeycode(t9_key)
}
class MatrixT9Keyboard {
+init()
+handleMatrixKeyPress(key)
+switchInputMode(mode)
+setTextOutputCallback(callback)
}
class SDL_Mouse {
+create()
+bind(group)
}
class SDL_Keyboard {
+create()
+bind(group)
}
lv_indev_t --> lv_indev_data_t : "读回调填充"
SDL_Mouse --> lv_indev_t : "注册读回调"
SDL_Keyboard --> lv_indev_t : "注册读回调"
T9Keyboard --> T9KeyMap : "使用映射表"
MatrixT9Keyboard --> T9KeyMap : "使用映射表"
```

**图表来源**
- [lv_indev.h:62-78](file://libs/lvgl/src/indev/lv_indev.h#L62-L78)
- [lv_indev.c:261-317](file://libs/lvgl/src/indev/lv_indev.c#L261-L317)
- [lv_indev_private.h:30-59](file://libs/lvgl/src/indev/lv_indev_private.h#L30-L59)
- [ui_app.cpp:55-81](file://src/ui/ui_app.cpp#L55-L81)
- [t9_keyboard.h:77-156](file://src/ui/common/t9_keyboard.h#L77-L156)
- [t9_keymap.h:49-88](file://src/ui/common/t9_keymap.h#L49-L88)
- [matrix_t9_keyboard.h:81-163](file://src/ui/common/matrix_t9_keyboard.h#L81-L163)

**章节来源**
- [lv_indev.h:62-78](file://libs/lvgl/src/indev/lv_indev.h#L62-L78)
- [lv_indev.c:261-317](file://libs/lvgl/src/indev/lv_indev.c#L261-L317)
- [lv_indev_private.h:30-59](file://libs/lvgl/src/indev/lv_indev_private.h#L30-L59)
- [ui_app.cpp:55-81](file://src/ui/ui_app.cpp#L55-L81)
- [t9_keyboard.h:16-156](file://src/ui/common/t9_keyboard.h#L16-L156)
- [t9_keymap.h:15-88](file://src/ui/common/t9_keymap.h#L15-L88)
- [matrix_t9_keyboard.h:16-163](file://src/ui/common/matrix_t9_keyboard.h#L16-L163)

### 显示设备适配
- SDL显示驱动
  - 创建窗口、设置标题、渲染器、纹理与缓冲区
  - 刷新回调flush_cb将LVGL绘制区域映射到SDL纹理并呈现
- LVGL配置
  - 颜色深度、默认刷新周期、DPI、渲染配置等
- 多分辨率与颜色格式
  - 通过LVGL颜色格式与stride对齐配置，适配不同像素格式
  - SDL纹理与LVGL缓冲区之间进行格式转换与更新

```mermaid
sequenceDiagram
participant LV as "LVGL"
participant Disp as "SDL显示驱动"
participant SDL as "SDL渲染器"
LV->>Disp : 请求刷新(区域/像素映射)
Disp->>Disp : 根据颜色格式转换/对齐
Disp->>SDL : 更新纹理/复制到渲染器
SDL-->>LV : 窗口呈现完成
```

**图表来源**
- [lv_sdl_window.c:250-437](file://libs/lvgl/src/drivers/sdl/lv_sdl_window.c#L250-L437)
- [lv_conf.h:29-95](file://lv_conf.h#L29-L95)

**章节来源**
- [lv_sdl_window.c:250-437](file://libs/lvgl/src/drivers/sdl/lv_sdl_window.c#L250-L437)
- [lv_conf.h:29-95](file://lv_conf.h#L29-L95)

### 硬件抽象层设计
- 设备接口标准化
  - 显示：LVGL显示驱动接口，flush回调、缓冲区管理
  - 输入：LVGL输入设备接口，读回调、事件数据结构，**新增T9键盘接口**
  - 摄像头：业务层统一接口，后台线程与UI线程分离
  - **T9键盘：提供统一的键盘事件处理接口，支持多种输入模式**
- 设备状态管理
  - 采集线程状态、识别开关、冷却时间、UI刷新节流
  - **T9键盘状态：输入模式、候选词、按键重复计数、大小写状态**
- 错误处理机制
  - OpenCV异常捕获、未知异常捕获、视频流断连重连、队列满丢弃
  - **T9键盘错误处理：按键映射错误、输入法切换失败、候选词查找异常**

**章节来源**
- [face_demo.cpp:537-547](file://src/business/face_demo.cpp#L537-L547)
- [face_demo.cpp:315-340](file://src/business/face_demo.cpp#L315-L340)
- [face_demo.cpp:472-478](file://src/business/face_demo.cpp#L472-L478)
- [t9_keyboard.cpp:39-48](file://src/ui/common/t9_keyboard.cpp#L39-L48)
- [matrix_t9_keyboard.cpp:26-36](file://src/ui/common/matrix_t9_keyboard.cpp#L26-L36)

## 依赖关系分析
- 应用入口依赖UI层与业务层
- UI层依赖LVGL显示驱动与输入设备
- 业务层依赖OpenCV、SQLite3与事件总线
- LVGL输入设备依赖读回调与事件数据结构
- **T9键盘系统依赖映射表和字典，提供输入法支持**

```mermaid
graph LR
main["src/main.cpp"] --> ui["src/ui/ui_app.cpp"]
ui --> sdl["libs/lvgl/src/drivers/sdl/lv_sdl_window.c"]
ui --> indev["libs/lvgl/src/indev/*.c"]
ui --> biz["src/business/face_demo.cpp"]
ui --> t9["src/ui/common/t9_keyboard*.cpp"]
biz --> opencv["OpenCV(VideoCapture/GStreamer)"]
biz --> sqlite["SQLite3"]
t9 --> keymap["src/ui/common/t9_keymap.cpp"]
t9 --> matrix["src/ui/common/matrix_t9_keyboard.cpp"]
```

**图表来源**
- [main.cpp:213-238](file://src/main.cpp#L213-L238)
- [ui_app.cpp:34-94](file://src/ui/ui_app.cpp#L34-L94)
- [face_demo.cpp:557-694](file://src/business/face_demo.cpp#L557-L694)
- [lv_sdl_window.c:95-239](file://libs/lvgl/src/drivers/sdl/lv_sdl_window.c#L95-L239)
- [lv_indev.c:261-317](file://libs/lvgl/src/indev/lv_indev.c#L261-L317)
- [t9_keyboard.cpp:1-200](file://src/ui/common/t9_keyboard.cpp#L1-L200)
- [t9_keymap.cpp:1-128](file://src/ui/common/t9_keymap.cpp#L1-L128)
- [matrix_t9_keyboard.cpp:1-200](file://src/ui/common/matrix_t9_keyboard.cpp#L1-L200)

**章节来源**
- [main.cpp:213-238](file://src/main.cpp#L213-L238)
- [ui_app.cpp:34-94](file://src/ui/ui_app.cpp#L34-L94)
- [face_demo.cpp:557-694](file://src/business/face_demo.cpp#L557-L694)

## 性能考虑
- 采集线程优化
  - 跳帧检测降低CPU占用，同时保持识别精度
  - 识别冷却与业务防抖减少重复写库
- UI刷新节流
  - UI刷新最小间隔约16ms，兼顾流畅度与CPU占用
  - **T9键盘UI更新优化：候选词缓存、UI重绘节流**
- 渲染与缓冲
  - LVGL颜色深度与stride对齐配置，减少格式转换成本
  - SDL纹理更新与Direct模式选择影响性能
- **T9键盘性能优化**
  - 字典查找算法优化，支持快速候选词匹配
  - 输入预测和缓存机制，提升输入响应速度

**章节来源**
- [face_demo.cpp:291-307](file://src/business/face_demo.cpp#L291-L307)
- [face_demo.cpp:514-527](file://src/business/face_demo.cpp#L514-L527)
- [lv_conf.h:128-167](file://lv_conf.h#L128-L167)

## 故障排查指南
- 摄像头/视频流问题
  - 管道参数不匹配：确认GStreamer管道与源格式一致
  - 断流重连：监控连续失败计数，超过阈值强制释放并重连
  - OpenCV异常：捕获异常并短暂休眠，避免刷屏
- UI黑屏/无响应
  - SDL窗口创建失败：检查lv_conf.h中SDL启用与依赖安装
  - 键盘绑定：确保键盘输入类型设置为KEYPAD并绑定UI组
- 数据库写入失败
  - 队列满丢弃：观察队列长度，必要时降低识别频率
  - 异常捕获：写库异常不会导致进程崩溃
- **T9键盘问题**
  - **按键无响应**：检查键码映射是否正确，验证事件处理函数是否被调用
  - **输入法不切换**：检查#键的键码映射，验证输入法切换逻辑
  - **候选词不显示**：检查字典初始化，验证T9输入处理逻辑
  - **硬件键盘事件未处理**：确认硬件键盘事件正确传递到T9键盘处理逻辑

**章节来源**
- [face_demo.cpp:223-240](file://src/business/face_demo.cpp#L223-L240)
- [face_demo.cpp:334-340](file://src/business/face_demo.cpp#L334-L340)
- [face_demo.cpp:537-547](file://src/business/face_demo.cpp#L537-L547)
- [ui_app.cpp:55-81](file://src/ui/ui_app.cpp#L55-L81)

## 结论
本指南提供了智能考勤系统硬件设备集成的完整路径：摄像头驱动适配与视频流处理、输入设备抽象与外设接入、显示设备多分辨率与颜色格式适配、以及硬件抽象层的设计要点。**新增的T9键盘系统为硬件部署提供了完整的键盘输入解决方案，支持多种输入模式和候选词选择功能。**通过事件总线与后台线程分离，系统在保证UI流畅的同时，实现了高效的人脸识别与考勤记录落库。

## 附录

### 硬件配置示例
- 显示配置
  - 分辨率：在UI层宏定义中设置SCREEN_W/SCREEN_H
  - 颜色深度：在LVGL配置中设置LV_COLOR_DEPTH
  - 刷新周期：在LVGL配置中设置LV_DEF_REFR_PERIOD
- 摄像头配置
  - GStreamer管道参数：根据实际源格式调整payload/width/height/colorimetry
  - USB摄像头：确保驱动与权限正确，OpenCV可正常打开设备
- 输入设备配置
  - 键盘/鼠标：UI层已创建并绑定到UI组
  - **T9键盘配置**：**硬件键盘键码映射、输入法模式设置、候选词显示配置**
  - 外设扩展：参考LVGL输入设备模板文件实现读回调

### **T9键盘系统配置**
- **文件结构**
  - t9_keyboard.h/cpp：T9键盘核心逻辑
  - t9_keymap.h/cpp：键盘映射表
  - matrix_t9_keyboard.h/cpp：矩阵键盘处理
- **编译配置**
  ```cmake
  # T9键盘模块
  add_library(t9_keyboard STATIC
      src/ui/common/t9_keyboard.cpp
      src/ui/common/t9_keymap.cpp
      src/ui/common/matrix_t9_keyboard.cpp
  )
  target_include_directories(t9_keyboard PRIVATE src/ui/common)
  target_link_libraries(your_target PRIVATE t9_keyboard)
  ```
- **基本使用**
  ```cpp
  // 初始化T9键盘
  T9Keyboard t9_kb;
  t9_kb.init();
  
  // 处理硬件键盘事件
  void handleHardwareKeyEvent(uint32_t pc_keycode, bool pressed) {
      if (!pressed) return;
      
      // 将PC键码转换为T9键码
      T9Key t9_key = keymap.findT9KeyByPCKeycode(pc_keycode);
      
      // 处理T9按键
      t9_kb.handleKeyPress(t9_key);
      
      // 获取当前文本和候选词
      std::string text = t9_kb.getCurrentText();
      auto candidates = t9_kb.getCandidates();
  }
  ```

**章节来源**
- [ui_app.cpp:20-27](file://src/ui/ui_app.cpp#L20-L27)
- [lv_conf.h:29-95](file://lv_conf.h#L29-L95)
- [face_demo.cpp:223-240](file://src/business/face_demo.cpp#L223-L240)
- [lv_port_indev_template.h:7-49](file://libs/lvgl/examples/porting/lv_port_indev_template.h#L7-L49)
- [lv_port_disp_template.h:6-57](file://libs/lvgl/examples/porting/lv_port_disp_template.h#L6-L57)
- [T9键盘集成指南.md:68-134](file://docs/markdowm/T9键盘集成指南.md#L68-L134)
- [t9_keyboard.h:16-156](file://src/ui/common/t9_keyboard.h#L16-L156)
- [t9_keymap.h:15-88](file://src/ui/common/t9_keymap.h#L15-L88)
- [matrix_t9_keyboard.h:16-163](file://src/ui/common/matrix_t9_keyboard.h#L16-L163)