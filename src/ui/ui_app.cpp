...
// --- 维护菜单列表点击事件 ---
static void sys_op_list_cb(lv_event_t *e) {
    lv_event_code_t code = lv_event_get_code(e);
    // 支持按键(ENTER)和点击
    if (code != LV_EVENT_CLICKED && !(code == LV_EVENT_KEY && lv_event_get_key(e) == LV_KEY_ENTER)) {
        if (code == LV_EVENT_KEY && lv_event_get_key(e) == LV_KEY_ESC) {
            load_menu_screen(); // ESC 返回
        }
        return;
    }

    lv_obj_t *btn = (lv_obj_t*)lv_event_get_target(e);
    const char *txt = lv_list_get_button_text(obj_sys_list, btn);
    
    if (std::strcmp(txt, "Clear Records") == 0) {
        // 创建 v9 Msgbox
        lv_obj_t * mbox = lv_msgbox_create(NULL);
        lv_msgbox_add_title(mbox, "Confirm");
        lv_msgbox_add_text(mbox, "Delete ALL logs?");
        
        // 添加按钮并绑定事件
        lv_obj_t * b_yes = lv_msgbox_add_footer_button(mbox, "Yes");
        lv_obj_t * b_no = lv_msgbox_add_footer_button(mbox, "No");
        
        // 绑定回调，将 mbox 作为 user_data 传入，以便关闭它
        lv_obj_add_event_cb(b_yes, op_clear_confirm_cb, LV_EVENT_CLICKED, mbox);
        lv_obj_add_event_cb(b_no, mbox_close_event_cb, LV_EVENT_CLICKED, mbox);
        
        lv_obj_center(mbox);
    }
    else if (std::strcmp(txt, "Factory Reset") == 0) {
        lv_obj_t * mbox = lv_msgbox_create(NULL);
        lv_msgbox_add_title(mbox, "DANGER");
        lv_msgbox_add_text(mbox, "Wipe ALL data?");
        
        // 红色背景警示
        lv_obj_set_style_bg_color(mbox, lv_palette_main(LV_PALETTE_RED), 0);
        
        lv_obj_t * b_yes = lv_msgbox_add_footer_button(mbox, "Reset");
        lv_obj_t * b_no = lv_msgbox_add_footer_button(mbox, "Cancel");
        
        lv_obj_add_event_cb(b_yes, op_reset_confirm_cb, LV_EVENT_CLICKED, mbox);
        lv_obj_add_event_cb(b_no, mbox_close_event_cb, LV_EVENT_CLICKED, mbox);
        
        lv_obj_center(mbox);
    }
    else if (std::strcmp(txt, "Exit App") == 0) {
        request_exit();
    }
}

// --- 创建维护菜单页面 ---
static void create_sys_ops_screen() {
    screen_sys_ops = lv_obj_create(NULL);
    lv_obj_set_style_bg_color(screen_sys_ops, lv_color_black(), 0);
    
    lv_obj_t *title = lv_label_create(screen_sys_ops);
    lv_label_set_text(title, "System Maintenance");
    lv_obj_set_style_text_color(title, lv_color_white(), 0);
    lv_obj_align(title, LV_ALIGN_TOP_MID, 0, 10);
    
    obj_sys_list = lv_list_create(screen_sys_ops);
    lv_obj_set_size(obj_sys_list, 200, 240);
    lv_obj_align(obj_sys_list, LV_ALIGN_CENTER, 0, 10);
    lv_obj_set_style_bg_color(obj_sys_list, lv_color_hex(0x222222), 0);
    lv_obj_set_style_border_width(obj_sys_list, 0, 0);
    
    // 填充选项
    const char* opts[] = {"Clear Records", "Factory Reset", "Exit App"};
    for(const char* opt : opts) {
        lv_obj_t *btn = lv_list_add_button(obj_sys_list, LV_SYMBOL_SETTINGS, opt);
        lv_obj_add_event_cb(btn, sys_op_list_cb, LV_EVENT_ALL, nullptr);
    }
}

static void load_sys_ops_screen() {
    if (!screen_sys_ops) create_sys_ops_screen();
    
    // 重新构建 Group 焦点逻辑
    lv_group_remove_all_objs(g_keypad_group);
    
    uint32_t cnt = lv_obj_get_child_cnt(obj_sys_list);
    for(uint32_t i=0; i<cnt; i++) {
        lv_obj_t *btn = lv_obj_get_child(obj_sys_list, i);
        if(lv_obj_check_type(btn, &lv_button_class)) { // 过滤掉 List 可能的 Label
             lv_group_add_obj(g_keypad_group, btn);
             if(i==0) lv_group_focus_obj(btn);
        }
    }
    
    lv_screen_load(screen_sys_ops);
}
